(function () {
  ...
  function saveData(maxAge, expireKey, indexKey) {
    ...
    console.log("[Search] Saved index data in localStorage under key:", indexKey);
  }
  ...
  function search(query) {
    ...
    var loop = function (i) {
      ...
      // Remove Markdown/HTML tags from postContent
        ...
        keywords.forEach(function (keyword) {
          ...
          if (indexTitle >= 0 || indexContent >= 0) {
            ...
            var start =
              indexContent < 11
                ? 0
                : postContent.lastIndexOf(" ", indexContent - 10);
            var end =
              start === 0
                ? 70
                : postContent.indexOf(" ", indexContent + keyword.length + 60);
            if (end === -1) {
            ...
            var matchContent =
              ...
              handlePostContent
                ...
                .replace(regEx, function (word) {
                  return (
                    "<em class=\"search-keyword\">" + word + "</em>"
                  );
                }) +
              "...";
  ...
  function init(config, vm) {
    ...
    // Force fresh index for debugging
    var isExpired = true;
    ...
    console.log("[Search] Building index for paths:", paths);
    ...
    paths.forEach(function (path) {
      ...
      Docsify.get(vm.router.getFile(path), false, vm.config.requestHeaders).then(function (result) {
        ...
        if (++count === len) {
          ...
          console.log("[Search] Finished indexing custom paths.");
        }
      });
    });
  }
  ...
  var install = function (hook, vm) {
    ...
    if (Array.isArray(opts)) {
      ...
      // Retain the value for opts.paths; it may be an array or a string.
    ...
    function initSearch() {
      ...
      // Build the search UI and bind events.
      init$1(CONFIG, vm);
      // If we're in auto-index mode, rebuild the index on doneEach.
    ...
    hook.mounted(function () {
      if (typeof CONFIG.paths === "string") {
        console.log("[Search] CONFIG.paths is a string. Attempting to fetch JSON from:", CONFIG.paths);
        // Fetch the JSON file for custom paths.
        fetch(CONFIG.paths)
          ...
          .then(function (pathsArray) {
            ...
            init$1(CONFIG, vm);  // setup UI
            init(CONFIG, vm);    // force custom index build immediately
          })
          .catch(function (error) {
            console.error("[Search] Failed to load search paths from JSON:", error);
            // Fallback to auto-index if fetch fails.
            CONFIG.paths = "auto";
            initSearch();
          });
      } else {
