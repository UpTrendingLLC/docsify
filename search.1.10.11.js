(function () {
  ...
  function init(config, vm) {
    ...
    // Force a fresh index for debugging
  ...
  // Modified tpl() function: Append the search UI to the sidebar if available.
  function tpl(defaultValue) {
    ...
    // Instead of placing before an <aside>, we now look for a .sidebar element.
    var sidebar = Docsify.dom.find(".sidebar");
    if (sidebar) {
      Docsify.dom.append(sidebar, el);
      console.log("[Search] Appended search UI to .sidebar");
    } else {
      // Fallback: insert before <aside> if .sidebar is missing.
