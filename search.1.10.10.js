(function () {
  ...
  function init(config, vm) {
    ...
    // Force a fresh index (for debugging, we clear any stored index)
  ...
  var install = function (hook, vm) {
    ...
    function initSearch() {
      ...
      init$1(CONFIG, vm);
      // Only in auto-index mode do we re-build the index on doneEach.
    ...
    hook.mounted(function () {
      // Clear any existing search index to force a rebuild with our custom paths.
      var indexKey = resolveIndexKey(CONFIG.namespace);
      localStorage.removeItem(indexKey);
    ...
    hook.doneEach(function () {
      update(CONFIG, vm);
      // In doneEach we only want to auto-index if no custom paths are used.
      if (CONFIG.paths === "auto") {
        init(CONFIG, vm);
      }
    });
