(function () {
   ...
   *
   * This processes a provided string and looks for tokens in the format
   * of `:name[=value]` and then converts it to an object and returns it.
   *
  ...
  var install = function (hook, vm) {
    ...
    function initSearch() {
      // Call init$1 to build the search UI and bind events.
      init$1(CONFIG, vm);
      // If we're in auto-index mode, build the index on every doneEach.
    ...
    hook.mounted(function () {
      if (typeof CONFIG.paths === "string") {
        // If paths is a string (like '/paths.json'), fetch the JSON file.
        fetch(CONFIG.paths)
          ...
          .catch(function (error) {
            console.error("Failed to load search paths from JSON:", error);
    ...
    // In the doneEach hook, only update UI and, if in auto mode, rebuild the index.
