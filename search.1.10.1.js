      ...
      if (token.type === 'heading' && token.depth <= depth) {
        ...
        if (tokenIndex === 0) {
          ...
          if (slug && index[slug]) {
              ...
              console.error("Error: slug or index[slug] is undefined", slug, index[slug]);
          }
...
function search(query) {
  ...
  var loop = function (i) {
    var post = data[i];
    if (!post) continue; // Skip undefined entries
    var matchesScore = 0;
    var resultStr = "";
    var handlePostTitle = "";
    ...
    if (!postTitle && !postContent) continue; // Skip empty content
