...
function genIndex(path, content, router, depth) {
    ...
    tokens.forEach(function (token, tokenIndex) {
        if (token.type === "heading" && (token.depth === 2 || token.depth === 3)) {
            ...
            // Standardize structure: Ensure every indexed item has a consistent key
            if (!index[slug]) {
            ...
            index[slug].title = title;
        } else {
            // Ensure there's always a default entry for each page
            if (tokenIndex === 0) {
      ...
      //console.log("Indexing:", path, token.type, token.text);
      ...
      if (postTitle) {
        keywords.forEach(function (keyword) {
          ...
          if (indexTitle >= 0 || indexContent >= 0) {
            ...
            if (end === -1) {
              end = postContent.length; // Ensure it captures until the end if needed
            }
...
function init(config, vm) {
    ...
    // Force fresh index if storage versioning changes
    ...
    paths.forEach(function (path) {
        ...
        Docsify.get(vm.router.getFile(path), false, vm.config.requestHeaders)
