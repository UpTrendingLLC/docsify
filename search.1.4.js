(function () {
   ...
   * ```
   *
   * @param {string}   str   The string to parse.star
   *
   * @return {object}  The original string and parsed object, { str, config }.
   */
  function getSnippet(content, index, keyword) {
    if (!content) return "";
    ...
    for (let word of words) {
        if (length + word.length + 1 > 50) break; // Ensure snippet doesn't exceed 50 chars
        snippet += (snippet ? " " : "") + word; // Append words with space
        length += word.length + 1;
    }

    return snippet.replace(new RegExp(keyword, "gi"), function (word) {
    ...
    tokens.forEach(function (token, tokenIndex) {
      if (token.type === "heading" && (token.depth === 2 || token.depth === 3)) {
        ...
        if (config.id) {
          ...
          if (token.depth === 3) {
            index[slug] = { slug: slug, title: text, body: "" };
          }
        }
      ...
      if (postTitle) {
        keywords.forEach(function (keyword) {
          ...
          if (indexTitle >= 0 || indexContent >= 0) {
            ...
            start = indexContent < 31 ? 0 : indexContent - 30;
            ...
            var matchContent =
                handlePostContent &&
                getSnippet(handlePostContent, start, end).replace(regEx, function (word) {
                  return '<em class="search-keyword">' + word + "</em>";
                });
...
function doSearch(value) {
  ...
  var shownResults = []; // Track displayed results to avoid duplicates

  var html = "";
matchs.forEach(function (post) {
  // Remove query parameters from post.url
  var cleanUrl = post.url.split('?')[0];

  if (shownResults.indexOf(cleanUrl) === -1) { // Check if result has already been displayed
    shownResults.push(cleanUrl); // Add to the displayed list
    html +=
